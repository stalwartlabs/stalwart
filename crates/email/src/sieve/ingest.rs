...
impl SieveScriptIngest for Server {
    ...
    async fn sieve_script_load(
        ...
        match Sieve::from_bytes(script_object.script.as_ref()) {
            Ok(script) => Ok(Some(CompiledScript {
                script,
                name: script_object.name.as_str().into(),
                version,
            })),
            Err(_) => self
                .sieve_script_recompile(account_id, document_id, version, &archive, script_object)
                .await
                .map(Some),
        }
    }
    ...
    async fn sieve_script_recompile(
        ...
        let new_script_object = SieveScript {
            ...
            script: script.to_bytes().map_err(|err| {
                trc::StoreEvent::UnexpectedError
                    .caused_by(trc::location!())
                    .reason(err)
