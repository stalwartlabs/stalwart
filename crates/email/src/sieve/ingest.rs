...
impl<'x> SieveHandler<'x> {
    fn file_into(
        ...
        if let Some(message) = self.messages.get_mut(message_id) {
            ...
            message.did_file_into |= is_file_into;
    ...
    fn action(&mut self, _: &Context<'x>, action: SieveAction<'_>) -> Reply<()> {
        match action {
            SieveAction::Keep { flags, message_id } => {
                ...
                self.file_into(INBOX_ID, flags, message_id, false);
            }
            SieveAction::Discard => {
            ...
            SieveAction::FileInto {
                ...
                match target_id {
                    ...
                    None => {
                        ...
                        self.file_into(target_id, flags, message_id, true);
                    }
                }
            }
                ...
                Ok(Status::Pending) => {
                    let input = match handler.pending.take() {
                        ...
                        Some(PendingWork::CreateMailbox {
                            ...
                            handler.file_into(target_id, flags, message_id, true);
