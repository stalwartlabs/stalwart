...
pub fn exec_tokenize(ctx: PluginContext<'_>) -> trc::Result<Variable<'static>> {
    let mut v = ctx.arguments;
    let (urls, urls_without_scheme, emails) = match v[1].to_string().as_ref() {
        "words" => return Ok(tokenize_words(&v[0]).into_owned()),
...
pub fn exec_domain_part(ctx: PluginContext<'_>) -> trc::Result<Variable<'static>> {
    let v = ctx.arguments;
